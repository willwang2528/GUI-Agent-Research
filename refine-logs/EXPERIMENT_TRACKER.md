...
| S1-M1-02c | M1 | 重建 outcome-blind Stage A measurement | synthetic fixtures | full-block frame/stream/raw/A0/A1/exposure/role schemas、schema-first validator、negative matrix | MUST | IMPLEMENTATION STABLE / FRESH AUDIT PENDING | full-block Stage A implementation 79/79 PASS，Ajv 2020 strict 27/27 PASS；X58 raw-support laundering、X59 late child freeze、X60 future role activation 已有 executable rejection。仍须独立复核，且生产外部 authority 全部 fail closed |
...
| S1-M7-03 | M7 | Round 5 implementation battle | full-block Stage A + bounds mechanics | executable counterexamples X36–X61 | MUST | PARTIAL ACCEPT / SCIENCE REJECT | bounds synthetic mechanics 获独立 ACCEPT；Stage A X58–X60 实现稳定但待 fresh replay；所有 production、freeze、Block A 与 Step 1 结论继续 REJECT |
