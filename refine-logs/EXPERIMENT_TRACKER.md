...
| S1-M1-02c | M1 | 重建 outcome-blind Stage A measurement | synthetic fixtures | coordinator + rolling prefix + A0/A1 + omission + block-barrier schemas、schema-first validator、negative matrix | MUST | PARTIAL IMPLEMENTATION / NOT READY | 11 个 closed schemas、29 个 one-damage fixtures、full suite 47/47；single-unit 与 block entry 均 fail closed。full-block manifest、A0-only adjudication、permanent role ledger 与 whole-block leak invalidation 尚未实现 |
